...
{
	const PIMAGE_NT_HEADERS64 nt_headers = portable_executable::GetNtHeaders(DrkPub_BEDAISY);
	...
	do
	{
		...
		memcpy(local_image_base, DrkPub_BEDAISY, nt_headers->OptionalHeader.SizeOfHeaders);
		...
		for (auto i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i)
		{
			...
			memcpy(local_section, reinterpret_cast<void*>(reinterpret_cast<uint64_t>(DrkPub_BEDAISY) + current_image_section[i].PointerToRawData), current_image_section[i].SizeOfRawData);
		}
